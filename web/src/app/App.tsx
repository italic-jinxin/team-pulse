...
import{api,cleanError,fetchDashboard,queryKeys,type DashboardData,type SyncJob}from'../lib/api';
...
export function App(){
 ...
 const notifiedRisks=useRef(new Set<string>(JSON.parse(localStorage.getItem('teampulse.notifiedRisks')||'[]')));
 ...
 useEffect(()=>{
  ...
  highRisks.slice(0,3).forEach(risk=>{
   ...
   notify('High risk detected',`${risk.repository||'Repository'}${risk.pr_number?` #${risk.pr_number}`:''}: ${risk.reason||'Needs attention'}`);
  });
  localStorage.setItem('teampulse.notifiedRisks',JSON.stringify(Array.from(notifiedRisks.current).slice(-200)));
 ...
 useEffect(()=>{
  ...
  const timer=window.setInterval(()=>{
   ...
   if(day===notifications.weeklyReminderDay&&time===notifications.weeklyReminderTime&&weeklyReminderKey.current!==key){
    weeklyReminderKey.current=key;
    notify('Weekly report reminder','Generate your TeamPulse weekly engineering summary.');
   }
  },30_000);
  return()=>window.clearInterval(timer);
 },[notifications.weeklyReminder,notifications.weeklyReminderDay,notifications.weeklyReminderTime]);

 const handleSync=()=>{
  if(!connected||repoCount===0){setPage('Settings');return;}
  syncTracked.mutate();
 };

 return <Shell page={page} setPage={setPage} connected={connected} riskCount={riskCount} openPRCount={openPRCount} repoCount={repoCount} lastSync={lastSync} refreshing={dashboard.isFetching} syncing={syncTracked.isPending||activeJob.data?.status==='running'||activeJob.data?.status==='pending'} activeJob={activeJob.data} syncSelection={syncSelection} error={(dashboard.error||syncTracked.error)?cleanError(dashboard.error||syncTracked.error):''} onRefresh={()=>dashboard.refetch()} onSync={handleSync}>
  {dashboard.isLoading||!data?<Loading/>:<Page page={page} data={data} activeJobId={activeJobId} syncSelection={syncSelection} onJobStart={(id,selection)=>{setActiveJobId(id);setSyncSelection(selection);}} onNavigate={setPage}/>}
 </Shell>;
}

function Page({page,data,activeJobId,syncSelection,onJobStart,onNavigate}:{page:PageName;data:DashboardData;activeJobId:string;syncSelection:string[];onJobStart:(id:string,selection:string[])=>void;onNavigate:(page:PageName)=>void}){
