...
export function Team({ rows, activity }: { rows: Row[]; activity: Row[] }) {
  ...
  const selected = useMemo(
    () => members.find((row) => row.login === selectedLogin) || members[0],
    [members, selectedLogin],
  );
  const selectedActivity = useMemo(
    () => (selected ? activityFor(selected.login, activity, range) : []),
    [selected?.login, activity, range],
  );
  const summary = useMemo(
    () => (selected ? buildSummary(selected, selectedActivity) : null),
    ...
    <div className="split">
      <section className="card">
        <div className="card-header">
          <h2>Team Members</h2>
          <span>
            {members.length} active · {rangeLabel(range)}
          </span>
        </div>
        <div className="filters team-filters">
          <Dropdown
            value={range}
            onChange={setRange}
            ariaLabel="Team activity time range"
            options={[
              { value: '1', label: 'Today' },
            ...
            {members.map((member) => (
              <li
                className={
                  ...
                  (selected?.login === member.login ? 'selected' : '')
                }
                key={member.login}
                onClick={() => setSelectedLogin(String(member.login))}
              >
                <div className="member-top">
                  <span className="member-title">{member.login}</span>
                  <span className="pill blue">{focusLabel(member)}</span>
                </div>
                <div className="member-desc">
                  {member.commits || 0} commits · {member.pull_requests || 0} PRs ·{' '}
                  {member.reviews || 0} reviews · {member.activity_count || 0} total events · last
                  active {ago(member.last_active_at)}
                </div>
              </li>
            ))}
          </ul>
        ) : (
          <Empty title="No members found" text="No contributor activity in this time range." />
        )}
      </section>
      <div className="grid">
        <section className="card">
          ...
          {selected && summary ? (
            <div className="detail-panel member-summary">
              <div className="activity-row">
                <div className="avatar">{initials(selected.login)}</div>
                <div className="activity-copy">
                  <h3>{summary.focus}</h3>
                  <p>{summary.recommendation}</p>
                </div>
              </div>
              <div className="kpi-strip member-kpis">
                <div className="mini-kpi">
                  <strong>{selected.commits || 0}</strong>
                  <span>Commits</span>
                </div>
                <div className="mini-kpi">
                  <strong>{selected.pull_requests || 0}</strong>
                  <span>PRs</span>
                </div>
                <div className="mini-kpi">
                  <strong>{selected.reviews || 0}</strong>
                  <span>Reviews</span>
                </div>
                <div className="mini-kpi">
              ...
              <div className="summary-grid">
                <div className="insight-box">
                  <span>Cadence</span>
                  <strong>{summary.avgPerActiveDay} events / active day</strong>
                  <p>{summary.lastActive}</p>
                </div>
                <div className="insight-box">
                  <span>Collaboration mode</span>
                  <strong>{summary.collaboration}</strong>
                  <p>
                    {selected.reviews || 0} reviews across {selected.pull_requests || 0} PR events.
                  </p>
                </div>
              </div>
              <div className="summary-section">
                <h3>Primary repositories</h3>
                {summary.topRepos.length ? (
                  <div className="repo-chips">
                    {summary.topRepos.map((repo) => (
                      <span key={repo.name}>
                        <b>{repo.name}</b>
                        <i>{repo.count}</i>
                      </span>
                  ...
                  <p>No repository activity in this range.</p>
                )}
              </div>
              <div className="summary-section">
                <h3>Activity mix</h3>
              ...
              <div className="summary-section">
                ...
                {summary.recent.length ? (
                  ...
                  <p>No recent events in this range.</p>
                )}
              </div>
            </div>
          ) : (
            <Empty title="Select a member" text="Choose a contributor for details." compact />
          )}
        </section>
        <section className="card">
          <div className="card-header">
          ...
          <div className="card-body stack">
            <div className="tag-row">
              <span className="pill blue">Reviews · {selected?.reviews || 0}</span>
              <span className="pill neutral">Pull requests · {selected?.pull_requests || 0}</span>
              <span className="pill neutral">Commits · {selected?.commits || 0}</span>
              <span className="pill neutral">Repos · {summary?.repoCount || 0}</span>
            </div>
            <p className="settings-note">
              {summary?.collaboration || 'Select a contributor to inspect collaboration signals.'}
            </p>
          </div>
        </section>
      </div>
...
function deriveMembers(rows: Row[], activity: Row[], range: string): MemberStats[] {
  ...
  filtered.forEach((item) => {
    ...
    if (!current.last_active_at || String(item.occurred_at) > String(current.last_active_at))
      current.last_active_at = item.occurred_at;
    byLogin.set(login, current);
  });
  const derived = Array.from(byLogin.values()).filter(
    (member) => range === 'all' || member.activity_count > 0,
  );
  if (filtered.length === 0 && range === 'all') {
    return rows.map((row) => ({
      ...
      activity_count:
        Number(row.commits || 0) + Number(row.pull_requests || 0) + Number(row.reviews || 0),
    }));
  }
  return derived.sort(
    (a, b) =>
      Number(b.activity_count || 0) - Number(a.activity_count || 0) ||
...
function activityFor(login: unknown, activity: Row[], range: string) {
  return activity
    .filter(
      (item) => String(item.actor || '') === String(login) && inRange(item.occurred_at, range),
    )
...
function buildSummary(member: MemberStats, events: Row[]): MemberSummary {
  ...
  const activeDays = new Set(
    events.map((event) => String(event.occurred_at || '').slice(0, 10)).filter(Boolean),
  ).size;
  ...
  return {
    ...
    topRepos: Array.from(repoCounts.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 4)
      .map(([name, count]) => ({ name, count, percent: Math.round((count / maxRepo) * 100) })),
    mix: ['Commits', 'Pull requests', 'Reviews', 'Other'].map((label) => {
      const count = typeCounts.get(label) || 0;
      return { label, count, percent: Math.round((count / maxType) * 100) };
    }),
    ...
    lastActive: member.last_active_at
      ? `Last active ${ago(member.last_active_at)}.`
...
function collaborationMode(commits: number, prs: number, reviews: number) {
  ...
  if ((commits + prs) / total >= 0.7 && reviews <= 1)
    return 'Implementation-heavy contributor; review load appears low.';
...
function summaryCopy(member: Row, eventCount: number) {
  const commits = Number(member.commits || 0),
    prs = Number(member.pull_requests || 0),
    reviews = Number(member.reviews || 0);
  if (eventCount === 0)
    return `${member.login} has no captured GitHub events in this time range. Expand the range or sync more repositories.`;
  if (reviews > commits + prs)
    return `${member.login} is primarily contributing through reviews. This is useful for flow health; check whether review load is concentrated on this person.`;
  if (prs >= 2 && reviews === 0)
    return `${member.login} is driving implementation work, but review participation is low in this range. Consider balancing review ownership.`;
  if (commits >= 5)
    return `${member.login} is actively shipping code. Look at recent repositories and PR signals to understand current delivery focus.`;
