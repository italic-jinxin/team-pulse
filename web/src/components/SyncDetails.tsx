...
export function SyncDetails({
  ...
  const progressLabel = [
    info.stage,
    info.currentRepo,
    info.repoProgress !== '—' ? info.repoProgress : '',
  ]
    .filter(Boolean)
    .join(' · ');
  return (
    <section className="syncpanel" aria-live="polite">
      ...
      <div
        className="progressbar sync-progressbar"
        aria-label={`Sync progress: ${progressLabel}`}
        aria-valuemin={0}
        aria-valuemax={100}
      ...
      {repositories.length > 0 && (
        <details className="syncdetails" open={!terminal}>
          <summary>{repositories.length} selected repositories</summary>
          <ol>
            {repositories.map((repo, index) => (
              <li className={repo === info.currentRepo ? 'active' : ''} key={repo}>
                <span>{index + 1}</span>
                {repo}
              </li>
            ))}
...
function parseSyncInfo(message: string, repositories: string[], progress: number) {
  ...
  return {
    ...
    repoProgress:
      index >= 0
        ? `${index + 1} of ${repositories.length}`
        : repositories.length
          ? `1 of ${repositories.length}`
          : '—',
    progress,
  };
}
...
function extractRepo(message: string, repositories: string[]) {
  return (
    repositories.find((repo) => message.includes(repo)) ||
    message.match(/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+/)?.[0] ||
    ''
  );
}
...
function syncFailureDetail(error: string) {
  ...
  if (repo)
    return `Sync failed in ${repo}. ${error}. Check repository access, token scopes, or GitHub API rate limits, then retry.`;
